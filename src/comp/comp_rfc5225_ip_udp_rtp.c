...
static bool rohc_comp_rfc5225_ip_udp_rtp_check_profile(const struct rohc_comp *const comp,
                                                   const struct net_pkt *const packet)
	__attribute__((warn_unused_result, nonnull(1, 2)));

/* check whether a packet belongs to a context */
static bool rohc_comp_rfc5225_ip_udp_rtp_check_context(const struct rohc_comp_ctxt *const context,
                                                   const struct net_pkt *const packet,
                                                   ...
                                                   size_t *const cr_score)
{
	...
	*cr_score = 0; /* Context Replication is not defined for ROHCv2 IP/UDP profile */
	...
	assert(remain_len >= (sizeof(struct udphdr) + sizeof(struct rtphdr)));
	...
	{
		const struct rtphdr *const rtp = (struct rtphdr *) remain_data;
		const uint16_t new_rtp_ssrc = rohc_ntoh16(rtp->ssrc);

		if(rfc5225_ctxt->rtp_ssrc != new_rtp_ssrc)
		{
			rohc_comp_debug(context, "  not same SSRC");
			goto bad_context;
		}
		rohc_comp_debug(context, "  same SSRC source port");
                                           ...
                                           size_t *const payload_offset)
{
	...
	*payload_offset = 0;
	...
	if(!rohc_comp_rfc5225_ip_udp_rtp_detect_changes(context, uncomp_pkt, payload_offset))
	{
	...
	*packet_type = rohc_comp_rfc5225_ip_udp_rtp_decide_pkt(context);
	...
	{
		ret = rohc_comp_rfc5225_ip_udp_rtp_code_IR_pkt(context, &uncomp_pkt->outer_ip,
	...
	{
		ret = rohc_comp_rfc5225_ip_udp_rtp_code_co_repair_pkt(context, &uncomp_pkt->outer_ip,
		                                                  rohc_remain_data, rohc_remain_len,
		                                                  *payload_offset);
		if(ret < 0)
		{
	...
	{
		ret = rohc_comp_rfc5225_ip_udp_rtp_code_CO_pkt(context, &uncomp_pkt->outer_ip,
		                                           rohc_remain_data, rohc_remain_len,
                                                ...
                                                const size_t rohc_pkt_max_len)
{
	...
	/* add static chain */
	ret = rohc_comp_rfc5225_ip_udp_rtp_static_chain(context, ip, rohc_remain_data,
	                                            rohc_remain_len);
	if(ret < 0)
	{
	...
	/* add dynamic chain */
	ret = rohc_comp_rfc5225_ip_udp_rtp_dyn_chain(context, ip, rohc_remain_data,
	                                         rohc_remain_len);
	if(ret < 0)
	{
                                                ...
                                                const size_t payload_offset)
{
	...
	ret = rohc_comp_rfc5225_ip_udp_rtp_irreg_chain(context, ip, rohc_remain_data,
	                                           rohc_remain_len);
	if(ret < 0)
	{
                                                    ...
                                                    const size_t rohc_max_len)
{
	...
	if(rohc_max_len < rtpudp_static_len)
	{
 ...
 *                        -1 in case of error
 */
/* TODO uncomment for story 52 and complete with all rtp fields
                                                 ...
                                                 const size_t rohc_max_len)
{
	...
	rtp_dynamic->reorder_ratio = rtp->reorder_ratio;
...
const struct rohc_comp_profile rohc_comp_rfc5225_ip_udp_rtp_profile =
{
	...
	.check_profile  = rohc_comp_rfc5225_ip_udp_rtp_check_profile,
	.check_context  = rohc_comp_rfc5225_ip_udp_rtp_check_context,
